...
public class SolanaController : MonoBehaviour
{
    ...
    public void Awake()
    {
        ...
        _testBtn.onClick.AddListener(()=>{
                                         ...
                                         var rest = GetTestEventWrapper(context).Result;
                                         $"TestEnd.".ToLog();
                                     });
    ...
    private async Task TestEvent(SynchronizationContext context)
    {
        ...
        var response = await solanaClient.GetLatestBlockHashAsync();
