...
namespace Solnet.Programs.Abstract
{
    ...
    public abstract class BaseClient
    {
        ...
        protected ProgramAccountsResultWrapper<List<T>> GetProgramAccounts<T>(string programAddress, List<MemCmp> filters,
            int? dataSize = null, Commitment commitment = Commitment.Finalized) where T : class
        {
            RequestResult<List<AccountKeyPair>> res = RpcClient.GetProgramAccounts(programAddress, commitment, dataSize, filters);
        ...
        /// <typeparam name="T">The specified type.</typeparam>
        /// <returns>A <see cref="ResultWrapper{T, T2}"/> containing the RPC response and the deserialized account if successful.</returns>
        protected MultipleAccountsResultWrapper<List<T>> GetMultipleAccounts<T>(List<string> accountAddresses,
            Commitment commitment = Commitment.Finalized) where T : class
        {
            RequestResult<ResponseValue<List<AccountInfo>>> res = RpcClient.GetMultipleAccounts(accountAddresses, commitment);
        ...
        /// <typeparam name="T">The specified type.</typeparam>
        /// <returns>A <see cref="ResultWrapper{T, T2}"/> containing the RPC response and the deserialized account if successful.</returns>
        protected AccountResultWrapper<T> GetAccount<T>(string accountAddress,
            Commitment commitment = Commitment.Finalized) where T : class
        {
            RequestResult<ResponseValue<AccountInfo>> res = RpcClient.GetAccountInfo(accountAddress, commitment);
